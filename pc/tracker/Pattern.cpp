...
/* For private use by the PatternSequencerFileLoader */
class TrackFileLoader : public FileLoader
{
public:
  TrackFileLoader(struct Pattern *pat, int t) : FileLoader(0), pattern(pat), t(t) {}
...
size_t TrackFileLoader::load(SDL_RWops *file, size_t chunksize)
{
  size_t maxread = 0;
      ...
      case SubChunkID::Data:
      {
        ...
        for (int tr=0; tr < pattern->len; tr++)
        {
          ...
          if (a <= 0x7f) // positive
          {
            ...
            /* TODO: REMOVE THIS HACK */
            ::tracker->instruments[pr->instr - 1].used++; // update instrument metadata
          }
          else
          {
            if ( a & ( 1 << CBIT_NOTE ) )
...
size_t TrackFileLoader::save(SDL_RWops *file)
{
...
PatternFileLoader::PatternFileLoader(struct PatternMeta *patterns) : 
  FileLoader(ChunkID::Pattern), patterns(patterns)
{}

size_t PatternFileLoader::load(SDL_RWops *file, size_t chunksize)
{
  size_t maxread = 0;
  uint8_t idx = 0;
  bool idx_loaded = false;
  ...
  while (maxread < chunksize)
  {
    ...
    switch (subchunkid)
    {
      ...
      case SubChunkID::Track:
      {
        ...
        TrackFileLoader tfl(&patterns[idx].p, 0);
...
size_t PatternFileLoader::save(SDL_RWops *file)
{
  // PATTERNS
...
PatternSequencerFileLoader::PatternSequencerFileLoader(struct PatternSequencer *patseq) :
  FileLoader(ChunkID::PatternSequencer), patseq(patseq)
{}

size_t PatternSequencerFileLoader::load(SDL_RWops *file, size_t chunksize)
{
  size_t maxread = 0;
  uint8_t idx = 0;
  bool idx_loaded = false;
...
size_t PatternSequencerFileLoader::save(SDL_RWops *file)
{
  uint8_t byte;
  uint16_t word;
  uint16_t chunklen = 0;
